...
function recoverUnclosed(toks, ctx) {
    for (let i = 0; i < toks.length; i++) {
        ...
        if (d === undefined || d.kind !== 'delim' || d.char === '~' || !d.canOpen || d.count === 0)
            continue;
        if (d.origCount < 2)
            continue; // only ** / __ — see the fallback guards below
        ...
        if (d.char !== '_') {
            for (let j = i + 1; j < extent; j++) {
                ...
                if (cand === undefined || cand.kind !== 'delim')
                    continue;
                if (cand.char !== d.char || cand.count !== d.count)
                    continue;
                partner = j;
                break;
            }
        ...
        if (partner !== -1) {
            ...
            if (children.length > 0) {
                ...
                ctx.diag.repair('emphasis-auto-closed', `'${d.char.repeat(d.origCount)}' pair rejected by flanking rules (punctuation adjacency); matched them anyway`, d.line);
