...
const OUT = 'src/model-weights.ts';
...
function main() {
  ...
  // Emitted as a TypeScript module, not JSON. `import ... with { type: 'json' }`
  // is Node 22+, and the package supports Node 18; a plain module also survives
  // every bundler without a JSON loader.
  const model = {
 ...
 * it claims to support and through every bundler.
 *
...
export const MODEL_WEIGHTS: ModelFile = ${JSON.stringify(model, null, 2)};
`,
  );
  console.log(`\nwrote ${OUT}`);
}
